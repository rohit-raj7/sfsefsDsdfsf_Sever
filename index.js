...
const reconnectGracePeriods = new Map(); // Map of callId -> { timeoutId, disconnectedPeerId, reconnectingAt }
...
io.on('connection', (socket) => {
  ...
  // Self-Healing DB reconciliation on client connect/reconnect
  async function _syncUserBusyStateWithDatabase(userId) {
    if (!userId) return;
    try {
      // Check if there is a truly active call in the DB
      ...
      if (activeCheck.rows.length === 0) {
        // No active call in database. Heal memory and database states!
        if (busyUsers.has(userId)) {
          busyUsers.delete(userId);
          console.log(`[SELF-HEAL] Purged stale busy memory for user ${userId}`);
        }
        io.emit('listener_busy_status', { listenerUserId: userId, busy: false });
        await Listener.clearBusyByUserId(userId).catch(() => { });
      } else {
        // True active call exists. Ensure in-memory busy map is correct!
        const activeCallId = activeCheck.rows[0].call_id;
        busyUsers.set(userId, activeCallId);
        io.emit('listener_busy_status', { listenerUserId: userId, busy: true });
        console.log(`[SELF-HEAL] Validated active busy state for user ${userId} (Call: ${activeCallId})`);
        await Listener.setBusyByUserId(userId).catch(() => { });
      }
    } catch (err) {
      console.error('[SELF-HEAL] Failed to sync busy status with database:', err.message);
    }
  }
  ...
  function _emitToTargetSockets(target, event, payload) {
    const socketIds = _socketIdList(target);
  ...
  function _handleCallReconnect(peerId, targetSocket) {
    if (!peerId) return;
    for (const [callId, graceData] of reconnectGracePeriods.entries()) {
      if (String(graceData.disconnectedPeerId) === String(peerId)) {
        clearTimeout(graceData.timeoutId);
        reconnectGracePeriods.delete(callId);
        console.log(`[SOCKET] User/Listener ${peerId} reconnected to call ${callId} within grace period`);
        
        const timerData = activeCallTimers.get(callId);
        if (timerData) {
          const isCaller = String(timerData.callerId) === String(peerId);
          ...
          if (otherPeerId) {
            ...
            if (otherSockets) {
              _emitToTargetSockets(otherSockets, 'call:reconnected', { callId });
            }
          }
        }
        targetSocket.emit('call:reconnected', { callId });
        break;
      }
    }
  }
  ...
  socket.on('user:join', (data) => {
    ...
    _handleCallReconnect(userId, socket);
  });
  ...
  socket.on('listener:join', async (listenerUserId) => {
    ...
    // SELF-HEAL: Sync presence state with DB
    _syncUserBusyStateWithDatabase(listenerUserId);

    _handleCallReconnect(listenerUserId, socket);
  });
  ...
  socket.on('random:call_invite', (data) => {
    ...
    if (targetSocketId) {
      _emitToTargetSockets(targetSocketId, 'random:call_invite', {
        channelName,
        callerName,
        callerAvatar,
      });
  ...
  socket.on('random:match_closed', (data) => {
    ...
    const targetSocketId = connectedUsers.get(targetUserId) || listenerSockets.get(targetUserId);
    if (targetSocketId) {
      _emitToTargetSockets(targetSocketId, 'random:match_closed', {});
  ...
  socket.on('call:accept', async (data) => {
    ...
    if (callerSocketId) {
      _emitToTargetSockets(callerSocketId, 'call:accepted', {
        callId,
  ...
  socket.on('call:reject', (data) => {
    ...
    const callerSocketId = connectedUsers.get(callerId);
    if (callerSocketId) {
      _emitToTargetSockets(callerSocketId, 'call:rejected', {
        callId,
        ...
        try {
          ...
          if (!callData) {
            ...
            channelUsers.forEach(uid => {
              ...
              if (sid) _emitToTargetSockets(sid, 'call:connected', { callId, channelName, maxAllowedSeconds: 0, serverStartTimeMs });
            });
            return;
          }
          ...
          channelUsers.forEach(uid => {
            const sid = connectedUsers.get(uid);
            if (sid) {
              _emitToTargetSockets(sid, 'call:connected', {
                callId,
                channelName,
                maxAllowedSeconds,
                serverStartTimeMs,
          ...
          if (maxAllowedSeconds > 0) {
            ...
            const timerId = setTimeout(async () => {
              ...
              channelUsers.forEach(uid => {
                const sid = connectedUsers.get(uid);
                if (sid) {
                  _emitToTargetSockets(sid, 'call:ended', {
                    callId,
                    channelName,
  ...
  socket.on('call:end', (data) => {
    ...
    if (callId && reconnectGracePeriods.has(callId)) {
      clearTimeout(reconnectGracePeriods.get(callId).timeoutId);
      reconnectGracePeriods.delete(callId);
    }

    // Dynamic participant resolution to guarantee complete busy state purging
    const resolvedParticipantIds = new Set();
    if (endedBy) resolvedParticipantIds.add(String(endedBy));
    if (otherUserId) resolvedParticipantIds.add(String(otherUserId));

    if (endTimerData) {
      if (endTimerData.callerId) resolvedParticipantIds.add(String(endTimerData.callerId));
      if (endTimerData.listenerUserId) resolvedParticipantIds.add(String(endTimerData.listenerUserId));
    }

    const pending = pendingCalls.get(callId);
    if (pending) {
      if (pending.initiatorUserId) resolvedParticipantIds.add(String(pending.initiatorUserId));
      if (pending.calleeUserId) resolvedParticipantIds.add(String(pending.calleeUserId));
      if (pending.listenerUserId) resolvedParticipantIds.add(String(pending.listenerUserId));
    }

    // Un-busy all resolved participants instantly
    for (const uid of resolvedParticipantIds) {
      _clearBusyForCall(uid);
    }
    ...
    const notifyCallEnded = (targetUserId, extra = {}) => {
      ...
      const targetSocketId = connectedUsers.get(targetUserId) || listenerSockets.get(targetUserId);
      const socketIds = _socketIdList(targetSocketId);
      if (socketIds.length === 0) return;
      for (const sid of socketIds) {
        if (notifiedSocketIds.has(sid)) continue;
  ...
  socket.on('user:app_state', (data) => {
    ...
    if (appState === 'background') {
      ...
      const listenerSocketSet = listenerSockets.get(userId);
      if (
        socket.listenerUserId === userId ||
        (listenerSocketSet && listenerSocketSet.has(socket.id))
      ) {
        _markListenerOffline(userId, 'background');
      }
    } else if (appState === 'foreground') {
      const existingSockets = connectedUsers.get(userId);
      const wasForeground = !!(existingSockets && existingSockets.has(socket.id));
      socket.userId = userId;
      if (!connectedUsers.has(userId)) connectedUsers.set(userId, new Set());
      connectedUsers.get(userId).add(socket.id);
      lastSeenMap.set(userId, Date.now());
  ...
  // Mark messages as read (seen) — WhatsApp-style
  // 1. Updates DB: is_read=TRUE, status='seen'
  // 2. Notifies ONLY the original sender of each message via their personal
  //    room (user_${senderId}) so their tick turns green. We do NOT emit to
  //    the chat room because:
  //    a) That would deliver the event twice to the sender (who is in the room)
  //    b) The receiver (who triggered chat:read) should never update their own ticks
  socket.on('chat:read', async (data) => {
    const { chatId } = data || {};
    if (!chatId || !socket.userId) return;

    try {
      // markAsRead returns [{message_id, sender_id}, ...] — only unread messages
      // sent by OTHER users (not the reader themselves)
      const updatedRows = await Message.markAsRead(chatId, socket.userId);

      // Broadcast generic read receipt to the whole chat room (for typing indicator
      // dismissal and other generic UI updates on the sender side)
      ...
      // Group message IDs by original sender
      const bySender = {};
      for (const row of updatedRows) {
        const sid = row.sender_id;
        if (!sid) continue; // skip rows with null sender (deleted users)
        if (!bySender[sid]) bySender[sid] = [];
        bySender[sid].push(row.message_id);
      }

      // Notify each original sender via their personal room ONLY
      // Using personal room avoids double delivery when sender is also in chat room
      for (const [senderId, messageIds] of Object.entries(bySender)) {
        ...
        // Personal room: sender receives this regardless of which screen they're on
        io.to(`user_${senderId}`).emit('message_status_updated', payload);
        console.log(`[SOCKET] chat:read: Notified sender ${senderId} — ${messageIds.length} msg(s) seen in chat ${chatId}`);
      }
    } catch (error) {
  ...
  socket.on('disconnect', () => {
    ...
    for (const [callId, timerData] of activeCallTimers.entries()) {
      ...
      if (isCaller || isListener) {
        ...
        console.log(`[SOCKET] disconnect: Initiating 45s grace period for call ${callId} (${isCaller ? 'caller' : 'listener'} disconnected)`);
        
        // Notify the other peer that their partner is reconnecting
        const otherPeerId = isCaller ? timerData.listenerUserId : timerData.callerId;
        if (otherPeerId) {
          const otherSockets = connectedUsers.get(otherPeerId) || listenerSockets.get(otherPeerId);
          if (otherSockets) {
        ...
        const timeoutId = setTimeout(() => {
          reconnectGracePeriods.delete(callId);
          console.log(`[SOCKET] Grace period expired for call ${callId}, ending call`);
          
          const latestTimerData = activeCallTimers.get(callId);
          if (latestTimerData) {
            clearTimeout(latestTimerData.timerId);
            activeCallTimers.delete(callId);
            ...
            _clearBusyForCall(latestTimerData.callerId, latestTimerData.listenerUserId);

            if (activeChannels.has(callId)) {
              activeChannels.delete(callId);
            }

            (async () => {
              try {
                const elapsed = Math.max(0, Math.round((disconnectedAt - latestTimerData.startedAt) / 1000));
                const billDuration = Math.min(elapsed, latestTimerData.maxAllowedSeconds);
                if (billDuration > 0) {
                  await billingFinalize({ callId, durationSeconds: billDuration });
                  console.log(`[SOCKET] disconnect timeout: Safety billing for call ${callId} (${billDuration}s)`);
                }
              } catch (e) {
                console.error(`[SOCKET] disconnect timeout: Safety billing error for call ${callId}:`, e.message);
              }
            })();
          }
        }, 45000);

        reconnectGracePeriods.set(callId, {
          timeoutId,
          disconnectedPeerId: userId,
          reconnectingAt: disconnectedAt
        });
      }
    }
    ...
    if (userId) {
      ...
      // PENDING CALLS: If this user was caller or callee who disconnected during ringing,
      // notify the other participant and clean up.
      for (const [callId, pending] of pendingCalls.entries()) {
      ...
      for (const [channelName, users] of activeChannels.entries()) {
        if (users.has(userId)) {
          // Check if this channel is in a grace period
          if (reconnectGracePeriods.has(channelName)) {
            // Keep the user in the active channel since they are in a grace period.
            // We do not tear down the channel yet.
            continue;
          }
          ...
          users.forEach(otherUid => {
            if (otherUid !== userId) {
              ...
              if (otherSid) {
                _emitToTargetSockets(otherSid, 'call:ended', {
                  callId: channelName,
                  endedBy: userId,
      ...
      const socketsForUser = connectedUsers.get(userId);
      if (socketsForUser && socketsForUser.has(socket.id)) {
        if (presenceTimeouts.has(userId)) {
          clearTimeout(presenceTimeouts.get(userId));
        }
...
async function startServer() {
  try {
    ...
    // PRODUCTION SAFETY: Clear all stale is_busy flags and fail stuck active calls on startup.
    // If the server crashed or restarted, listeners may be stuck as busy, and active calls stuck.
    try {
      ...
      const clearedCalls = await startupPool.query(
        `UPDATE calls SET status = 'failed', ended_at = CURRENT_TIMESTAMP 
         WHERE (status IN ('pending', 'ringing', 'initiated') AND created_at < NOW() - INTERVAL '2 minutes')
            OR (status = 'ongoing' AND started_at < NOW() - INTERVAL '1 hour')`
      );
      if (clearedCalls.rowCount > 0) {
        console.log(`🧹 Failed ${clearedCalls.rowCount} stuck active call(s) in DB on startup.`);
      }
    } catch (err) {
      console.error('⚠️  Failed to clear stale busy flags / stuck calls on startup:', err.message);
    }
